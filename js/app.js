    ...
    (() => {
        ...
        let bodyLock = (delay = 500) => {
            ...
            if (bodyLockStatus) {
                ...
                body.style.paddingRight = window.innerWidth - document.querySelector(".app-root").offsetWidth + "px";
            ...
            if (document.querySelector(".slider-board")) {
                ...
                new core(".slider-board", {
                    ...
                    on: {
                        reachEnd: () => {
                            setTimeout((() => {
                                document.querySelector(".slider-board__button-done").removeAttribute("disabled");
                                sliderContainer.classList.add("slider-ends");
                            }), 10);
                        }
                    }
                });
        ...
        window.addEventListener("load", (windowEvent => {
            ...
            buttonsControllTimeTags.forEach((button => {
                button.dataset.layoutControll;
                layoutControlls.set(button, {
                    layouts: Array.from(timeStampsLayoutTimeTags).concat(Array.from(calendarLayoutTimeTags), Array.from(usersLayoutTags)),
                    timeStampLayout: Array.from(timeStampsLayoutTimeTags).find((elem => elem.dataset.layoutTimeDisplay == button.dataset.layoutControll ? true : false)),
                    calendarCellsLayout: Array.from(calendarLayoutTimeTags).find((elem => elem.dataset.layoutCellsDisplay == button.dataset.layoutControll ? true : false)),
                    scrollRelativeBlocks: Array.from(calendarLayoutTimeTags).find((elem => elem.dataset.layoutCellsDisplay == button.dataset.layoutControll ? true : false)).dataset.scrollRelative?.split(",").map((elem => elem.trim())),
                    swipeRelativeBlocks: Array.from(calendarLayoutTimeTags).find((elem => elem.dataset.layoutCellsDisplay == button.dataset.layoutControll ? true : false)).dataset.swipeRelative?.split(",").map((elem => elem.trim())),
                    usersLayoutTags,
            ...
            for (const iterator of layoutControlls) iterator[0].addEventListener("click", (e => {
                ...
                for (const iteratorInner of layoutControlls) {
                    if (iteratorInner[1].calendarCellsLayout.hasAttribute("data-scroll")) {
                        ...
                        iteratorInner[1].scrollRelativeBlocks?.forEach((block => scrollReset(document.querySelector(block))));
                    }
                    if (iteratorInner[1].calendarCellsLayout.hasAttribute("data-swipe")) {
                        scrollReset(iteratorInner[1].calendarCellsLayout);
                        iteratorInner[1].swipeRelativeBlocks?.forEach((block => scrollReset(document.querySelector(block))));
                    }
                }
                if (iterator[1].calendarCellsLayout.hasAttribute("data-scroll")) {
                    let scrollRelativeBlocks = null;
                    if (iterator[1].scrollRelativeBlocks) scrollRelativeBlocks = iterator[1].scrollRelativeBlocks.map((block => ({
                ...
                if (iterator[1].calendarCellsLayout.hasAttribute("data-swipe")) {
                    ...
                    if (iterator[1].swipeRelativeBlocks) swipeRelativeBlocks = iterator[1].swipeRelativeBlocks.map((block => ({
                        elem: document.querySelector(block),
            ...
            taskEditableFieldList.forEach((field => {
                field.addEventListener("click", (e => {
                    ...
                    if (e.target.closest(".task-details__row--span-2")) e.target.closest(".task-details__row--span-2").classList.toggle("_edit");
                }));
            }));
            document.addEventListener("click", (clickEvent => {
                const targetElement = clickEvent.target;
                if (targetElement.closest(".event-block") || targetElement.closest(".event-block__mobile") || targetElement.closest(".event-day")) if (document.documentElement.classList.contains("task-details-open")) {
                    document.documentElement.classList.remove("task-details-open");
                    setTimeout((() => {
                        document.documentElement.classList.add("task-details-open");
                    }), 300);
                } else document.documentElement.classList.add("task-details-open"); else if (!targetElement.closest(".task-details") || targetElement.closest(".task-details__close")) document.documentElement.classList.remove("task-details-open");
                if (targetElement.closest(".controlls__filter")) document.documentElement.classList.add("filters-open"); else if (!targetElement.closest(".filters") || targetElement.closest(".filters__close")) document.documentElement.classList.remove("filters-open");
                if (targetElement.closest("#new-job-button")) document.documentElement.classList.add("new-job-open"); else if (!targetElement.closest(".new-job") || targetElement.closest(".new-job__close")) document.documentElement.classList.remove("new-job-open");
                if ((targetElement.closest(".controlls__items--time") || targetElement.closest(".controlls__items--type")) && targetElement.closest(".select__options")) if (targetElement.closest('[data-layout-controll="day"]') || targetElement.closest('[data-layout-controll="staff"]')) document.querySelector(".calendar-layout").classList.add("day"); else document.querySelector(".calendar-layout").classList.remove("day");
                if (targetElement.closest(".controlls__items--time") && targetElement.closest(".select__options")) if (targetElement.closest('[data-layout-controll="day"]')) document.querySelector(".controlls__items--type").style.display = ""; else document.querySelector(".controlls__items--type").style.display = "none";
                if (targetElement.closest(".slider-board__button-skip")) modules_flsModules.popup.close();
                if (targetElement.closest(".slider-board__button-done")) {
                    let sliderContainer = document.querySelector(".slider-board");
                    if (sliderContainer.classList.contains("slider-ends")) modules_flsModules.popup.close();
                }
                if (targetElement.closest(".date__text")) targetElement.closest(".date__text").querySelector("button").click();
        ...
        function layerScrollTemplateInit(scrollBlock, allowScrollX, allowScrollY, dependentBlocks = null) {
            ...
            let leftEnd = 0;
            let scrollX = 0;
            let scrollableX = allowScrollX ? scrollBlock.clientWidth - (window.innerWidth - scrollBlock.getBoundingClientRect().x - paddingRight) : 0;
            let topStart = 0;
            let topEnd = 0;
            let scrollY = 0;
            ...
            window.addEventListener("resize", (resizeEvent => {
                ...
                scrollableY = allowScrollY ? scrollBlock.clientHeight - (wrapperContainer.clientHeight - scrollBlock.getBoundingClientRect().y) : 0;
            }));
        ...
        function layerSwipeTemplateInit(swipeBlock, allowSwipeX, allowSwipeY, dependentBlocks = null) {
            let wrapperContainer = document.querySelector(".wrapper-container");
            let paddingRight = parseInt(getComputedStyle(wrapperContainer, true).paddingRight);
            if (swipeBlock.hasAttribute("data-swipe-y-stop")) allowSwipeY = false; else allowSwipeY = true;
            let leftStart = 0;
            let leftEnd = 0;
            let swipeX = 0;
            let swipeableX = allowSwipeX ? swipeBlock.clientWidth - (window.innerWidth - swipeBlock.getBoundingClientRect().x - paddingRight) : 0;
            let topStart = 0;
            let topEnd = 0;
            let swipeY = 0;
            let swipeableY = allowSwipeY ? swipeBlock.clientHeight - (wrapperContainer.clientHeight - swipeBlock.getBoundingClientRect().y) : 0;
            window.addEventListener("resize", (resizeEvent => {
                swipeableX = allowSwipeX ? swipeBlock.clientWidth - (window.innerWidth - swipeBlock.getBoundingClientRect().x - paddingRight) : 0;
                swipeableY = allowSwipeY ? swipeBlock.clientHeight - (wrapperContainer.clientHeight - swipeBlock.getBoundingClientRect().y) : 0;
            }));
            ...
            swipeBlock.addEventListener("touchmove", (function(e) {
                ...
                if (allowSwipeX) {
                    ...
                    if (diffX <= 0) swipeX = 0; else if (diffX > swipeableX) swipeX = -swipeableX; else swipeX = -diffX;
                }
                if (allowSwipeX) {
                    swipeYdiff = firstTouch.clientY;
                    let diffY = -(topEnd + swipeYdiff - topStart);
                    if (diffY <= 0) swipeY = 0; else if (diffY > swipeableY) swipeY = -swipeableY; else swipeY = -diffY;
                }
            ...
            swipeBlock.addEventListener("touchend", (function(e) {
                if (allowSwipeX) {
                    ...
                    if (leftEnd > 0) leftEnd = 0; else if (leftEnd <= -swipeableX) leftEnd = -swipeableX;
                }
                if (allowSwipeX) {
                    topEnd += swipeYdiff - topStart;
                    if (topEnd > 0) topEnd = 0; else if (topEnd <= -swipeableY) topEnd = -swipeableY;
                }
            }));
        }
        ...
        function setDayLayoutRange(arrayValue) {
            ...
            document.querySelector(".calendar-layout__wrapper");
            ...
            document.querySelector(".time-stamp.time-stamp--day");
        ...
        function DayLayoutRangeCorrect() {
            ...
            let fullWidthWrapperCalculate = userLayout.offsetWidth + Array.from(cellList).reduce(((sum, el) => sum + Number.parseInt(window.getComputedStyle(el).getPropertyValue("min-width"))), 0);
            if (fullWidthWrapperCalculate < fullWidthWrapper) calendarLayoutInner.style.width = "100%"; else calendarLayoutInner.style.width = "";
        }
